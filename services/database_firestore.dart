...
class DatabaseService{
  ...
  static late FirebaseFirestore dataBaseEditors;
  static late CollectionReference fileEditors;

  DatabaseService({required uid_m, required dataBaseEditors_m}){
    uid = uid_m;
    dataBaseEditors = dataBaseEditors_m;
    fileEditors = dataBaseEditors.collection('f_editors');
  }

  Future<void> addFileEditor(FileEditor fileEditor) async {
    try {
      await dataBaseEditors.runTransaction((transaction) async {
    ...
    } catch (e) { print('Error adding FileEditor to Firestore: $e');}
  }
  ...
  Future<String> uploadFile(FileEditor fileEditor) async {
    ...
    try {
      DocumentReference<Map<String, dynamic>> documentReference = await fileEditors
          .doc(uid).collection('language').doc(language).collection('files').add({
                                                                                  'file_name': fileName,
                                                                                  'language': language,
                                                                                   'code': code
      });
  ...
  Future<void> deleteFileEditor(FileEditor fileEditor, String doc_id) async {
    ...
    } catch (e) {print('Error deleting FileEditor from Firestore: $e');}
  }
  ...
  Future updateFile(FileEditor fileEditor, String _code, String doc_id) async {
    ...
    } catch (e) { print('Error updating FileEditor at Firestore: $e'); }
  }
}